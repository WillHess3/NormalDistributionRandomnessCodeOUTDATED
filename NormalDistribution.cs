...
public static class NormalDistribution {
    ...
    public static float RandomOverNormalDistribution(float mean, float standardDeviation, float rand0to1) {
        float p = ((float)Math.Truncate(rand0to1 * 10000)) / 10000f;
        ...
        while (!ZTable.ContainsKey(((float)Math.Truncate(p * 10000)) / 10000f) && p < 1) {
            p += .0001f;
        ...
        if (smallP) {
            ZTable.TryGetValue((float)Math.Truncate(p * 10000) / 10000f, out z);
            z = -z;
        } else {
            ZTable.TryGetValue((float)Math.Truncate(p * 10000) / 10000f, out z);
        }
